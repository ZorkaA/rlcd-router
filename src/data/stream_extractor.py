...
def extract_activations(
    ...
    **kwargs: Any,
    ...
    """Convenience functional interface for activation extraction with boundary validation."""
    effective_layer = tap_layer if tap_layer is not None else layer_n
    if effective_layer < 0 or effective_layer > 24:
        raise ValueError(f"layer_n={effective_layer} is out of bounds [0, 24].")
    if model is None:
        raise ValueError("Model must be provided.")
    if tokens is None:
        raise ValueError("Tokens must be provided.")
    cfg = StreamExtractorConfig(
        tap_layer=effective_layer,
        seq_len=seq_len,
        device=device,
        cache_flush_interval=cache_flush_interval,
        **kwargs,
    ...
    extractor = StreamExtractor(model, config=cfg, hooks=hooks)
    return extractor.stream_token_tensor(tokens)
...
def chunk_tokens(
    ...
    """Split 1D or 2D token tensor into fixed sequence length chunks."""
    if tokens.dim() > 1:
    ...
    for i in range(0, total_tokens, chunk_size):
        ...
        if chunk.numel() < chunk_size:
            ...
            if pad_token_id is not None:
                pad_len = chunk_size - chunk.numel()
                padding = torch.full((pad_len,), pad_token_id, dtype=chunk.dtype, device=chunk.device)
...
def stream_corpus(
    model: nn.Module,
    corpus: Union[torch.Tensor, Iterable[str]],
    ...
    **kwargs: Any,
    ...
    """Stream a token tensor or text corpus through the model."""
    cfg = StreamExtractorConfig(
        tap_layer=tap_layer,
        seq_len=seq_len,
        batch_size=batch_size,
    ...
    if isinstance(corpus, torch.Tensor):
        return extractor.stream_token_tensor(corpus)
    elif tokenizer is not None:
        return extractor.stream_text_corpus(corpus, tokenizer=tokenizer)
    else:
        raise ValueError("Must provide either a token tensor or text corpus with tokenizer.")


TokenStreamer = StreamExtractor
ActivationExtractor = StreamExtractor
